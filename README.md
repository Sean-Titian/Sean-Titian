...
- **[Review Sentiment Reliability](https://github.com/Sean-Titian/review-sentiment-reliability)** — a clean-room, synthetic-only reliability study for a rating-derived text proxy. Contract 5.0 retains pre-specified 5%/10%/20% queue sensitivity and freezes four rolling test horizons across a pre-specified zero-day reference and authored 14- and 30-day proxy-label-delay scenarios. Relative to zero delay, mean AP changes at 14 and 30 days are +0.002 and −0.009, and mean lift-at-10% changes are −0.091 and −0.050, across four correlated synthetic windows; these non-monotonic changes are descriptive, not confidence intervals or evidence that one delay is preferable. All 12 window-level and three pooled test-label-alignment placebo gates pass fixed heuristic bounds. Because label-availability timestamps were not observed and the rating-derived target may already be visible, the delays are plumbing stress scenarios—not measured SLAs or recommended embargoes; no real-data, causal, operational, or production-value claim is made.
...
Review evaluation reliability has now moved from roadmap to public evidence. Contract 5.0 keeps four non-overlapping test horizons fixed across a pre-specified zero-day reference and authored 14- and 30-day proxy-label-delay scenarios, assigns all 2,400 synthetic rows to train, validation, embargo, test, or future in every window, and refits using only eligible history. Its 12 window-level and three pooled test-label-alignment placebo gates pass, while zero delay reproduces the prior rolling-origin metrics exactly. Recurring text and entities still cross time, however, and the fixed delays are not observed label latency. This demonstrates as-of evaluation plumbing that can expose uncertainty—not a validated target, SLA, production benefit, or staffing recommendation.
